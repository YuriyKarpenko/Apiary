...
namespace Apiary.VM
{
	class test_properties
	{
		public static Func<object, object> CreateGetter(PropertyInfo property)
		{
			...
			var genericMethod = typeof(test_properties).GetMethod("CreateGetterGeneric");
		...
		public static Action<object, object> CreateSetter(PropertyInfo property)
		{
			...
			var genericMethod = typeof(test_properties).GetMethod("CreateSetterGeneric");
			MethodInfo genericHelper = genericMethod.MakeGenericMethod(property.DeclaringType, property.PropertyType);
		...
		public test_properties(Type type)
		{
			var Properties = type.GetProperties(BindingFlags.Public | BindingFlags.Instance)
						.Where(p => p.CanRead && !p.GetIndexParameters().Any())
						.AsEnumerable();
			PropertyGetters = Properties
				.ToDictionary(p => p.Name, p => CreateGetter(p));
